...
// payload.bins.forEach(ensureHeatData);
...
async function init() {
  ...
  try {
    ...
    renderBinsFromDB(payload); // ✅ ONLY DB RENDER
    createZones();
    animate();
  } catch (err) {
...
function createPalletRack(bays, depth, levels) {
  ...
  const PALLET_W = 1.2;
  const LEVEL_H = 2.0;
  const FIRST_BEAM_Y = 0.95; // 🔑 first load level (NO beam at floor)
...
//   await fetch("/api/bins/upload-excel/", {
//     method: "POST",
//     body: fd,
//   });
