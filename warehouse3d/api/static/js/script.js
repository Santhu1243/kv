...
const BINS_PER_SHELF = 2;

const MODES = {
  STOCK: "STOCK",
  ABC: "ABC",
  HEAT: "HEAT",
  PICKFACE_UTIL: "PICKFACE_UTIL",
  PICKFACE_RECO: "PICKFACE_RECO",
  REPLENISH: "REPLENISH",
  SLOT_PLAN: "SLOT_PLAN",
  BIN_UTIL: "BIN_UTIL",
};
...
function activateMode(mode) {
  ...
  switch (mode) {
    ...
    case MODES.PICKFACE_UTIL:
      applyPickfaceUtilizationOverlay();
      updateLegend("PICKFACE_UTIL");
      break;

    case MODES.PICKFACE_RECO:
      showPickfaceRecommendations();
      updateLegend("PICKFACE_RECO");
      break;

    case MODES.REPLENISH:
      applyReplenishmentOverlay();
      updateLegend("REPLENISH");
      break;

    case MODES.SLOT_PLAN:
      applySlotPlanVsActual();
      updateLegend("SLOT_PLAN");
      break;

    case MODES.BIN_UTIL:
      applyBinUtilizationOverlay();
      updateLegend("BIN_UTIL");
      break;
...
const EMPTY_BIN_COLOR = new THREE.Color("#89837eff"); // brown
const OCCUPIED_BIN_COLOR = new THREE.Color("#2ecc71"); // green
const PICKFACE_ASSIGNED_COLOR = "#2563eb"; // 🔵 Blue (assigned)
const PICKFACE_UNASSIGNED_COLOR = "#f59e0b"; // 🟡 Amber (unassigned)
// Replenishment – Clean & Modern
const REPLENISH_ALERT_COLOR = "#ef4444"; // 🔴 Red
const REPLENISH_OK_COLOR = "#22c55e"; // 🟢 Green
const REPLENISH_IGNORE_COLOR = "#cbd5e1"; // ⚪ Gray (optional)
// Bin Utilization
const BIN_EMPTY_COLOR = "#cbd5e1"; // ⚪ Gray
const BIN_UTILIZED_COLOR = "#22c55e"; // 🟢 Green
const BIN_OVER_COLOR = "#ef4444"; // 🔴 Red// PickFace Recommendation
const PICKFACE_BEST_COLOR = "#22c55e"; // 🟢 Best pick face
const PICKFACE_GOOD_COLOR = "#84cc16"; // 🟡 Good
const PICKFACE_AVOID_COLOR = "#cbd5e1"; // ⚪ Not recommended

const BIN_SLOT_HEIGHT = 1.4; // visual bin slot height
...
function setupUI() {
  ...
  document.getElementById("btnRegenerate").addEventListener("click", () => {
    if (flattenedBins && flattenedBins.length) {
  ...
  document.getElementById("btnStock")?.addEventListener("click", () => activateMode(MODES.STOCK));

  document.getElementById("btnABC")?.addEventListener("click", () => activateMode(MODES.ABC));

  document.getElementById("btnHeat")?.addEventListener("click", () => activateMode(MODES.HEAT));

  document.getElementById("btnPickfaceUtil")?.addEventListener("click", () => activateMode(MODES.PICKFACE_UTIL));

  document.getElementById("btnPickfaceReco")?.addEventListener("click", () => activateMode(MODES.PICKFACE_RECO));

  document.getElementById("btnReplenish")?.addEventListener("click", () => activateMode(MODES.REPLENISH));

  document.getElementById("btnSlotPlan")?.addEventListener("click", () => activateMode(MODES.SLOT_PLAN));

  document.getElementById("btnClear")?.addEventListener("click", () => activateMode(MODES.NONE));
  document.getElementById("btnReplenish")?.addEventListener("click", () => activateMode(MODES.REPLENISH));
  document.getElementById("btnBinUtil")?.addEventListener("click", () => activateMode(MODES.BIN_UTIL));
...
const OVERLAY_STOCK = "STOCK";

function applyStockOverview() {
  overlayMode = OVERLAY_STOCK;

  warehouse.traverse((obj) => {
    if (!obj.isMesh || !obj.userData?.bin) return;

    const bin = obj.userData.bin;
    const isOccupied = (bin.qty ?? 0) > 0;

    const color = isOccupied ? new THREE.Color("#2ecc71") : new THREE.Color("#cbd5e1");
    ...
    obj.userData._baseColor = color.clone();
  });

  updateLegend("STOCK");
}
function applyReplenishmentOverlay() {
  overlayMode = "REPLENISH";
  ...
  warehouse.traverse((obj) => {
    ...
    const qty = bin.qty ?? 0;
    const minQty = bin.min_qty ?? 0;

    let color;

    // Optional: ignore empty & unassigned bins
    if (qty === 0 && isBinEmpty(bin)) {
      color = new THREE.Color(REPLENISH_IGNORE_COLOR);
    }
    // 🔴 Needs replenishment
    else if (qty < minQty) {
      color = new THREE.Color(REPLENISH_ALERT_COLOR);
    }
    // 🟢 Stock OK
    else {
      color = new THREE.Color(REPLENISH_OK_COLOR);
    }
...
function updateLegend(mode) {
  ...
  if (mode === "STOCK") {
    legend.innerHTML = `
      <div class="legend-title">Stock Overview</div>

      <div class="legend-row">
        <span class="legend-color" style="background:#2ecc71"></span>
        <span>🟢 Occupied Bin</span>
      </div>
  ...
  if (mode === "ABC") {
    legend.innerHTML = `
      <div class="legend-title">ABC Classification</div>
      <div class="legend-row"><span class="legend-color a"></span> 🟢 A – Fast Moving</div>
      <div class="legend-row"><span class="legend-color b"></span> 🔵 B – Medium Moving</div>
      <div class="legend-row"><span class="legend-color c"></span> 🔴 C – Slow Moving</div>
    `;
    return;
  }
  if (mode === "PICKFACE_UTIL") {
    legend.innerHTML = `
    <div class="legend-title">Pick Face Utilization</div>

    <div class="legend-row">
      <span class="legend-color" style="background:${PICKFACE_ASSIGNED_COLOR}"></span>
      🔵  Assigned Pick Face
    </div>

    <div class="legend-row">
      <span class="legend-color" style="background:${PICKFACE_UNASSIGNED_COLOR}"></span>
      🟡  Unassigned Pick Face
    </div>
  `;
    return;
  }
  if (mode === "REPLENISH") {
    legend.innerHTML = `
    <div class="legend-title">Replenishment Status</div>

    <div class="legend-row">
      <span class="legend-color" style="background:${REPLENISH_ALERT_COLOR}"></span>
       🔴 Replenishment Required
    </div>

    <div class="legend-row">
      <span class="legend-color" style="background:${REPLENISH_OK_COLOR}"></span>
       🟢 Stock OK
    </div>

    <div class="legend-row">
      <span class="legend-color" style="background:${REPLENISH_IGNORE_COLOR}"></span>
     ⚪  Empty / Unassigned
    </div>
  `;
    return;
  ...
  if (mode === "BIN_UTIL") {
    legend.innerHTML = `
    <div class="legend-title">Bin Utilization</div>

    <div class="legend-row">
      <span class="legend-color" style="background:${BIN_UTILIZED_COLOR}"></span>
      Utilized
    </div>

    <div class="legend-row">
      <span class="legend-color" style="background:${BIN_EMPTY_COLOR}"></span>
      Empty
    </div>

    <div class="legend-row">
      <span class="legend-color" style="background:${BIN_OVER_COLOR}"></span>
      Over Capacity
    </div>
  `;
    return;
  }
  if (mode === "PICKFACE_RECO") {
    legend.innerHTML = `
    <div class="legend-title">PickFace Recommendation</div>

    <div class="legend-row">
      <span class="legend-color" style="background:${PICKFACE_BEST_COLOR}"></span>
     🟢 Best Pick 
    </div>

    <div class="legend-row">
      <span class="legend-color" style="background:${PICKFACE_GOOD_COLOR}"></span>
      🟡 Good Pick Location
    </div>

    <div class="legend-row">
      <span class="legend-color" style="background:${PICKFACE_AVOID_COLOR}"></span>
      ⚪ Avoid / Non-Optimal
    </div>
  `;
    return;
...
function renderSingleRackRow(rowId, shelvesMap, z) {
  ...
  // Create fixed shelf indexes: S1 → S{n}
  const shelfIds = Array.from({ length: shelvesPerRow }, (_, i) => i + 1);

  const columns = shelvesPerRow;

  const maxLevel = Math.max(...shelfIds.flatMap((s) => (shelvesMap[s] || []).map((b) => b.level)), 0);

  const rack = createPalletRack(columns, RACK_DEPTH, maxLevel + 1);
  rack.position.set(-(columns * PALLET_W) / 2, 0, z);
  warehouse.add(rack);

  const fullLayout = buildFullShelfLayout(shelfIds, shelvesMap, maxLevel);

  shelfIds.forEach((shelfId, colIndex) => {
    fullLayout[shelfId].forEach((bin) => {
      const group = new THREE.Group();
      ...
      const occupied = !!bin.occupied || !!bin.product || (bin.qty && bin.qty > 0);

      const boxHeight = occupied ? bin.height || BIN_SLOT_HEIGHT : BIN_SLOT_HEIGHT;

      const baseColor = occupied
        ? OCCUPIED_BIN_COLOR // 🟢 product assigned
        : EMPTY_BIN_COLOR; // 🟫 empty placeholder

      const boxMat = new THREE.MeshLambertMaterial({
        color: baseColor,
        transparent: true,
        opacity: occupied ? 0.95 : 0.8,
      });

      const box = new THREE.Mesh(new THREE.BoxGeometry(bin.width || 1.2, boxHeight, bin.depth || 1.2), boxMat);

      // sit on pallet
      box.position.y = 0.18 + boxHeight / 2;

      // REQUIRED for overlay reset
      box.userData._baseColor = baseColor.clone();
...
function handleHover(event) {
  ...
  if (hitMesh) {
    ...
    showHoverLabel(event.clientX, event.clientY, getBinLocationLabel(hoveredMesh.userData.bin));
  } else {
    clearHover();
    hideHoverLabel();
  }
}
function getBinLocationLabel(bin) {
  if (!bin) return "Bin";

  const r = bin.row ?? bin.row_id ?? "-";
  const s = bin.shelf ?? bin.shelf_id ?? "-";
  const l = bin.level ?? "-";
...
function updateRightPanel(d) {
  ...
  const capacity = d.capacity ?? 10000;
  const used = d.qty ?? 0;
  const available = Math.max(capacity - used, 0);

  if (used > capacity) {
    document.getElementById("pd_capacity").style.color = BIN_OVER_COLOR;
    document.getElementById("pd_used").style.color = BIN_OVER_COLOR;
  }
  ...
  const score = computePickfaceScore(d, stats);

  document.getElementById("pd_pickface_score").textContent = score.toFixed(2);

  document.getElementById("pd_pickface_score").style.color = score > 0.75 ? PICKFACE_BEST_COLOR : score > 0.5 ? PICKFACE_GOOD_COLOR : "#94a3b8";
...
function getPickfaceRecommendations(limit = 10) {
  ...
  warehouse.traverse((obj) => {
    ...
    // ✅ ONLY BOTTOM PICK FACES
    if (bin.level !== 0) return;

    // ❌ Ignore empty / placeholder bins (optional but recommended)
    if (isBinEmpty(bin)) return;

    const score = computePickfaceScore(bin, stats);
    bins.push({ mesh: obj, bin, score });
  });

  return bins.sort((a, b) => b.score - a.score).slice(0, limit);
}

function buildFullShelfLayout(shelfIds, shelvesMap, maxLevel) {
  const layout = {};
  ...
  shelfIds.forEach((shelfId) => {
    ...
    for (let level = 0; level <= maxLevel; level++) {
      // try to find DB bin
      const existing = shelvesMap[shelfId]?.find((b) => b.level === level);

      if (existing) {
        layout[shelfId].push(existing);
      } else {
        ...
        layout[shelfId].push({
          ...
          hits: 0,
          abc: "C",
          bin_code: `EMPTY-S${shelfId}-L${level}`,
          isPlaceholder: true,
        });
      }
    }
...
function isBinEmpty(d) {
  const hasQty = (d.qty ?? 0) > 0;
  const hasProduct = (Array.isArray(d.products) && d.products.length > 0) || !!d.product;

  return !hasQty && !hasProduct;
}

function applyPickfaceUtilizationOverlay() {
  overlayMode = "PICKFACE_UTIL";

  warehouse.traverse((obj) => {
    if (!obj.isMesh || !obj.userData?.bin) return;

    const empty = isBinEmpty(obj.userData.bin);

    const c = empty ? new THREE.Color(PICKFACE_UNASSIGNED_COLOR) : new THREE.Color(PICKFACE_ASSIGNED_COLOR);
...
function applyBinUtilizationOverlay() {
  overlayMode = "BIN_UTIL";
  ...
  warehouse.traverse((obj) => {
    ...
    const qty = bin.qty ?? 0;
    const capacity = bin.capacity ?? 10000; // safe default

    let color;

    if (qty === 0) {
      color = new THREE.Color(BIN_EMPTY_COLOR);
    } else if (qty > capacity) {
      color = new THREE.Color(BIN_OVER_COLOR);
    } else {
      color = new THREE.Color(BIN_UTILIZED_COLOR);
    }
...
function showPickfaceRecommendations(limit = 10) {
  overlayMode = MODES.PICKFACE_RECO;

  const recommendations = getPickfaceRecommendations(limit);

  const recommendedSet = new Set(recommendations.map((r) => r.mesh.uuid));

  warehouse.traverse((obj) => {
    if (!obj.isMesh || !obj.userData?.bin) return;

    // 🔝 TOP recommendations
    if (recommendedSet.has(obj.uuid)) {
      const rec = recommendations.find((r) => r.mesh.uuid === obj.uuid);

      const color = rec.score > 0.75 ? PICKFACE_BEST_COLOR : PICKFACE_GOOD_COLOR;
    ...
    // 🚫 Others (dim)
      ...
      obj.material.color.set(PICKFACE_AVOID_COLOR);
      obj.material.emissive.set(0x000000);
      obj.material.emissiveIntensity = 0;
    }
  });
